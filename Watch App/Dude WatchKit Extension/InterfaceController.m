...
@interface InterfaceController() <RowControllerDelegate> {
  NSArray *contacts;
...
- (void)configureTableWithContacts {
  ...
  for (NSInteger i = 0; i < self.table.numberOfRows; i++) {
    ...
    DUser *user = [contactsLeft objectAtIndex:i];
  ...
  for (NSInteger i = 0; i < self.table.numberOfRows; i++) {
    if (contactsRight.count > i) {
      RowController *row = [self.table rowControllerAtIndex:i];
      DUser *user = [contactsRight objectAtIndex:i];
